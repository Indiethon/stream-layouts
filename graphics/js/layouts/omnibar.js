...
NodeCG.waitForReplicants(targetData, pollData, rewardData, campaignData, charityLogo, marathonLogo, runDataArray, runDataActiveRun).then(() => {
	...
	function staticText(index) {
		if (index === 0)
			setOmnibarHtml(`<p class="is-single-line is-text-centered">Indiethon is raising money for <span style="color: #6D91CB;">Child's Play</span></p>`,)
		else
			setOmnibarHtml(`<p class="is-single-line is-text-centered">Donate at <span style="color: #6D91CB;">https://donate.indiethon.live</span></p>`)
	}

	// Host text.
	function hostText() {
		setOmnibarHtml(`<p class='is-single-line is-text-centered'>Your current host is <span style="color: #6D91CB;">${host}</span></p>`)
	}
	...
	function targetText(index) {
		if (index === 0)
			setOmnibarHtml(`<p class="is-multiline is-text-centered">Next Incentive: ${targetData.value.data[index].name}</p> <p class="is-multiline is-text-centered">$${targetData.value.data[index].targetAmount} / $${targetData.value.data[index].currentAmount}</p>`)
		else
			setOmnibarHtml(`<p class="is-multiline is-text-centered">Later Incentive: ${targetData.value.data[index].name}</p> <p class="is-multiline is-text-centered">$${targetData.value.data[index].targetAmount} / $${targetData.value.data[index].currentAmount}</p>`)
	}
